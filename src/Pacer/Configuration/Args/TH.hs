    ...
    qs =
      GRT.firstSuccessQ
        [ GRT.embedGitError gitDataFromGitQ,
          GRT.runGitInEnvDirQ [osstr|PACER_HOME|] gitDataFromGitQ,
          gitDataFromEnvQ
        ]
...
--
-- - PACER_MODIFIED: unix time like "1746055623"
-- - PACER_HASH: long hash
-- - PACER_SHORT_HASH: short hash
--
...
gitDataFromEnvQ = do
  ...
  h <- validateHash 40 <$> GRT.envValQ [osstr|PACER_HASH|]
  sh <- validateHash 7 <$> GRT.envValQ [osstr|PACER_SHORT_HASH|]
  pure $ liftA3 (,,) d h sh
  where
